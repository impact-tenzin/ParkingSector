			...
			function roundMinutes(minutes) {
				...
				function renderBookingMsg(data) {
					...
					if (data == "User does not exist" || data=="Cant authenticate"){
							...
							$('.msg').html(msg);
							setTimeout(function(){$('.signInBox').hide();}, 1500);
							bookingReqeust();
						}
				}
				...
				function ajaxCall(lat, lng) {
					...
					$.ajax({
						...
						success : function(data) {
							...
							var parkingsData = data.filter(function(item){return item.model == "FindParking.parkingmarker";});
							var featuresData = data.filter(function(item){return item.model == "FindParking.parkingfeatures";});
							var paymentMethodData = data.filter(function(item){return item.model == "FindParking.paymentmethod";});
